...
ARG COOLOPS_BUILD_COMMIT=unknown
RUN echo "${COOLOPS_BUILD_COMMIT}" > /app/.build_commit
...
HEALTHCHECK --interval=30s --timeout=8s --start-period=180s --retries=5 \
    CMD python -c "import os,urllib.request; urllib.request.urlopen('http://127.0.0.1:%s/healthz/' % os.environ.get('PORT', '80'), timeout=5)" || exit 1
