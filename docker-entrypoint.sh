...
if [[ "${COOLOPS_COMPOSE_STACK:-0}" == "1" ]]; then
  export PORT=80
fi
...
if [ -z "${DJANGO_SECRET_KEY:-}" ]; then
  echo "[cool-ops] HATA: DJANGO_SECRET_KEY üretilemedi."
...
if [ -n "${DATA_DIR:-}" ] && [ ! -w "$DATA_DIR" ]; then
  echo "[cool-ops] HATA: ${DATA_DIR} yazılamıyor — Docker Compose volume (gy_data:/data) bağlı mı?"
  exit 1
fi
...
echo "[cool-ops] migrate + collectstatic..."
...
if [ "${DJANGO_WHATSAPP_BRIDGE_WAIT_ON_START:-0}" = "1" ] && [ "${DJANGO_WHATSAPP_BRIDGE_CAN_SPAWN:-0}" != "1" ] && [ -n "${WHATSAPP_BRIDGE_URL:-}" ]; then
  echo "[cool-ops] WhatsApp köprüsü bekleniyor (${WHATSAPP_BRIDGE_URL})..."
  python manage.py wait_whatsapp_bridge --timeout "${DJANGO_WHATSAPP_BRIDGE_WAIT_TIMEOUT:-30}" \
    || echo "[cool-ops] UYARI: köprü henüz hazır değil — panel yine de başlıyor."
fi

echo "[cool-ops] daphne ${HOST}:${PORT}"
